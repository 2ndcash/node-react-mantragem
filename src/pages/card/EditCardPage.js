...
import { _editCardForm, _fetchCardData } from '../../actions';
import Upload from '../../components/common/Upload';
import { MainLayout } from '../../components/layout'

function EditCardPage({ touched, errors, form, values, setFieldValue }) {

  const head = () => {
    return (
      <Helmet key={Math.random()}>
        <title>Mantragem - Edit Card</title>
      </Helmet>
    );
  };
            ...
            <div className="card-body">
              ...
              <Form className="forms-sample">
                <div className="box-bg">
                  ...
                  <div className="form-group row">
                    <label className="col-sm-3 col-form-label">หมายเลข</label>
                    <div className="col-sm-9">
                      <Field name="number" placeholder="Number" className="form-control" type="text" />
                      {touched.number && errors.number && <label className="text-danger">{` `}{errors.number}</label>}
                    </div>
                  </div>
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    submitHandler: (id, data) => dispatch(_editCardForm(id, data))
  }
}
export default {
